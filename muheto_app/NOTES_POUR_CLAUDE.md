...
- **Toujours vérifier avant qu'elle sauvegarde** : c'est elle qui montre le résultat (capture d'écran ou texte collé) AVANT de faire `Ctrl+S`, pas l'inverse — c'est ce qui permet de repérer une erreur (accolade `}` mal placée en JSON, indentation YAML foireuse) avant qu'elle soit enregistrée. Ne jamais sauter cette vérification, même pour un changement qui semble trivial.
- Elle utilise **à la fois des captures d'écran et du texte copié-collé**, selon ce qui est le plus pratique sur le moment : captures pour les manipulations visuelles (VS Code, Firebase Console, l'app sur son téléphone), texte copié-collé quand la précision exacte du contenu compte plus que voir l'écran (JSON, code Dart). Les deux sont bienvenus, pas de préférence à imposer.
