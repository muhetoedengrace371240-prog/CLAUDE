  ...
  Future<void> _send() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(AppLocalizations.of(context).t('chat.sendFailed'))),
      );
    } finally {
        ...
        title: GestureDetector(
          ...
          child: Row(
            children: [
              ...
              // Badge Gold en direct (contrairement à l'aperçu dénormalisé
              // de l'Inbox, ici on interroge le profil live — un seul
              // utilisateur affiché, donc coût négligeable).
              StreamBuilder<UserModel?>(
                stream: _profileService.watchUser(widget.otherUserId),
                builder: (context, snapshot) {
                  if (snapshot.data?.isGoldActive != true) return const SizedBox.shrink();
                  return const Padding(
                    padding: EdgeInsets.only(left: 4),
                    child: GoldBadge(size: 15),
                  );
                },
              ),
            ],
            ...
            child: StreamBuilder<List<MessageModel>>(
              ...
              builder: (context, snapshot) {
                ...
                if (messages.isEmpty) {
                  return Center(
                    child: Text(
                      AppLocalizations.of(context).t('chat.sayHello'),
                  ...
                  Expanded(
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                        hintText: AppLocalizations.of(context).t('chat.typeMessage'),
