...
class _BusinessScreenState extends State<BusinessScreen> {
  ...
  Future<void> _handleMyPageTap() async {
    ...
    if (uid == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(AppLocalizations.of(context).t('business.loginRequired'))),
      );
      return;
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(AppLocalizations.of(context).t('business.title').toUpperCase()),
        actions: [
          IconButton(
            ...
            onPressed: () {
              Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (_) => const SearchScreen(initialTab: SearchResultTab.business),
                ),
              );
            },
          ),
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: Text(AppLocalizations.of(context).t('business.myPage'), style: const TextStyle(fontWeight: FontWeight.w700)),
      ),
    );
  }
}
