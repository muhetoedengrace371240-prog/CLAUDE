...
class _FeedScreenState extends State<FeedScreen> {
  ...
  Future<void> _handleLike(VideoModel video) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(loc.t('feed.loginToLike'))),
      );
    }
  }
          ...
          StreamBuilder<List<VideoModel>>(
            ...
            builder: (context, snapshot) {
              ...
              if (snapshot.hasError) {
                return Center(
                  child: Text(
                    AppLocalizations.of(context).t('feed.loadError'),
              ...
              if (videos.isEmpty) {
                return Center(
                  child: Text(
                    AppLocalizations.of(context).t('feed.empty'),
                    textAlign: TextAlign.center,
                    style: const TextStyle(color: Colors.white70),
                  ),
                );
  ...
  String _tabLabel(BuildContext context, FeedTab tab) {
    final loc = AppLocalizations.of(context);
    switch (tab) {
  ...
  Future<void> _send() async {
    ...
    try {
      ...
      final comment = CommentModel(
        id: '',
        userId: uid,
        username: author?.username ?? AppLocalizations.of(context).t('feed.defaultUsername'),
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(AppLocalizations.of(context).t('feed.commentFailed'))),
      );
    } finally {
          ...
          children: [
            ...
            Text(
              AppLocalizations.of(context)
                  .t('feed.commentsCount', {'count': '${widget.video.commentsCount}'}),
              ...
              child: StreamBuilder<List<CommentModel>>(
                ...
                builder: (context, snapshot) {
                  ...
                  if (comments.isEmpty) {
                    return Center(
                      child: Text(
                        AppLocalizations.of(context).t('feed.beFirstComment'),
                    ...
                    Expanded(
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          hintText: AppLocalizations.of(context).t('feed.addCommentHint'),
                          isDense: true,
                        ),
                        onSubmitted: (_) => _send(),
                      ),
