...
class BusinessService {
  ...
  // --- Suivi analytics (Brique 13) ---------------------------------------
  //
  // Chaque méthode fait un incrément atomique isolé (`FieldValue.increment`)
  // plutôt que de passer par `updateBusiness()` + `toFirestore()` — on évite
  // ainsi tout risque d'écraser un compteur avec une valeur périmée lue
  // avant l'incrément d'un autre utilisateur au même moment (deux personnes
  // pouvant ouvrir la fiche ou taper "Appeler" à la même seconde).

  /// À appeler une fois par ouverture de fiche (voir le flag anti-double-
  /// comptage dans `BusinessDetailScreen`), jamais à chaque rebuild.
  Future<void> registerBusinessView(String businessId) {
    return _businesses.doc(businessId).update({'viewsCount': FieldValue.increment(1)});
  }

  Future<void> registerCallClick(String businessId) {
    return _businesses.doc(businessId).update({'callClicksCount': FieldValue.increment(1)});
  }

  Future<void> registerWebsiteClick(String businessId) {
    return _businesses.doc(businessId).update({'websiteClicksCount': FieldValue.increment(1)});
  }

  Future<void> registerWhatsappClick(String businessId) {
    return _businesses.doc(businessId).update({'whatsappClicksCount': FieldValue.increment(1)});
  }
}
