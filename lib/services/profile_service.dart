...
class ProfileService {
  ...
  /// Lecture ponctuelle (non temps réel) du profil — utile au moment d'une
  /// action (ex: poster un commentaire) pour dénormaliser username/avatar/
  /// statut Gold actuels sans garder un listener ouvert en permanence.
  ...
  /// Vérifie si [username] est déjà pris par un autre utilisateur que
  /// [excludingUid] — utilisé par l'écran d'édition de profil avant
  /// d'enregistrer un changement de pseudonyme.
  Future<bool> isUsernameTaken(String username, {required String excludingUid}) async {
    final snap = await _users.where('username', isEqualTo: username).limit(1).get();
    if (snap.docs.isEmpty) return false;
    return snap.docs.first.id != excludingUid;
  }
