...
///   - isGoldMember: bool (l'utilisateur a un abonnement MUHETO Gold actif ou passé)
///   - goldExpirationDate: Timestamp? (date de fin de la période payée/essai en cours)
///   - createdAt: Timestamp
class UserModel {
  final String uid;
  ...
  /// Statut Gold réellement effectif à l'instant présent : `isGoldMember`
  /// ET la date d'expiration n'est pas dépassée (ou absente, pour un accès
  /// Gold accordé manuellement sans limite de durée). C'est CETTE valeur
  /// qu'il faut utiliser partout dans l'UI pour décider d'afficher le badge
  /// doré ou de débloquer les avantages — jamais `isGoldMember` seul, qui
  /// peut rester `true` en base après expiration tant qu'un job de
  /// désactivation ne l'a pas remis à `false`.
  bool get isGoldActive {
    if (!isGoldMember) return false;
    if (goldExpirationDate == null) return true;
